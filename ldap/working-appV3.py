...
def log_command(ssh, command, username, ip, action):
    ...
    stdout_output = stdout.read().decode().strip().replace('\n', '\\n')
    stderr_output = stderr.read().decode().strip().replace('\n', '\\n')
...
def run_command(ip, action, username, password):
    ...
    try:
        ...
        if action == "reboot":
            ...
            stdin, stdout, stderr, exit_code = log_command(ssh, list_services_cmd, username, ip, action)
            initial_services = [s.strip() for s in stdout.read().decode().splitlines() if s.strip()]
            with open(services_file, "w") as f:
                f.write("\n".join(initial_services))

            # Log reboot command
            log_command(ssh, "sudo shutdown -r now", username, ip, action)
            time.sleep(180)

            try:
                ssh.connect(ip, username=username, password=password, timeout=15)
                stdin, stdout, stderr, exit_code = log_command(ssh, "uptime -p || uptime", username, ip, action)
                uptime = stdout.read().decode().strip() or "N/A"

                # Log post-reboot services
                stdin, stdout, stderr, exit_code = log_command(ssh, list_services_cmd, username, ip, action)
                ...
                if missing_services:
                    ...
                    for service in missing_services:
                        ...
                        stdin, stdout, stderr, exit_code = log_command(ssh, restart_cmd, username, ip, action)
                ...
                app_logger.error(f"Post-reboot check failed: {str(e)}", exc_info=True,
                    extra={
                        'user': username,
                        'action': action,
                        'target': ip
            ...
            log_command(ssh, "sudo shutdown -h now", username, ip, action)
            time.sleep(2)
...
def check_cpu_usage(ssh, username, ip):
    ...
    stdin, stdout, stderr, exit_code = log_command(ssh, cmd, username, ip, 'health_check')
    try:
...
def check_memory_usage(ssh, username, ip):
    cmd = "free | awk '/Mem:/ {printf(\"%.2f\", $3/$2 * 100.0)}'"
    stdin, stdout, stderr, exit_code = log_command(ssh, cmd, username, ip, 'health_check')
    try:
        usage = float(stdout.read().decode().strip())
        return 'Yes' if usage < 80 else 'No'
...
def check_compliance(ssh, username, ip):
    try:
        ...
        if os_type in ["ubuntu", "debian"]:
            ...
            stdin, stdout, stderr, exit_code = log_command(ssh, "sudo apt-get -s upgrade", username, ip, 'compliance_check')
            output = stdout.read().decode().lower()
            return 'Yes' if '0 upgraded' in output else 'No'
            
        elif os_type in ["centos", "rhel"]:
            stdin, stdout, stderr, exit_code = log_command(ssh, "sudo yum check-update", username, ip, 'compliance_check')
            return 'No' if exit_code == 100 else 'Yes'
            
        elif os_type == "suse":
            stdin, stdout, stderr, exit_code = log_command(ssh, "sudo zypper --non-interactive list-updates", username, ip, 'compliance_check')
            output = stdout.read().decode().lower()
...
def check_selinux(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "getenforce", username, ip, 'health_check')
    try:
...
def check_reboot_required(ssh, username, ip):
    # Check Debian/Ubuntu
    stdout, _, _ = log_command(ssh, "[ -f /var/run/reboot-required ] && echo Yes || echo No", username, ip, 'health_check')
    if 'Yes' in stdout:
        return 'Yes'
    
    # Check RHEL/CentOS
    stdout, _, _ = log_command(ssh, "needs-restarting -r &>/dev/null; echo $?", username, ip, 'health_check')
    if stdout.strip() == '1':
        return 'Yes'
    
    # Check SUSE
    stdout, _, _ = log_command(ssh, "[ -f /var/run/reboot-needed ] && echo Yes || echo No", username, ip, 'health_check')
    return 'Yes' if 'Yes' in stdout else 'No'


def check_disk_usage(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "df -h | awk '$5 > 80 {print $1}'", username, ip, 'health_check')
    try:
...
def check_disk_inodes(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "df -i | awk '$5 > 80 {print $1}'", username, ip, 'health_check')
    try:
        return 'No' if len(stdout.read().decode().strip()) == 0 else 'Yes'
    except:
        return 'Error'
...
def check_load_average(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "cat /proc/loadavg | awk '{print $1, $2, $3}'", username, ip, 'health_check')
    try:
...
def check_firewall(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "sudo ufw status | grep 'Status: active'", username, ip, 'health_check')
    try:
...
def check_ssh_config(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "sudo sshd -t", username, ip, 'health_check')
    try:
...
def check_uptime(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "uptime -p || uptime", username, ip, 'health_check')
    try:
...
def get_kernel_version(ssh, username, ip):
    ...
    return stdout_output if exit_code == 0 else 'Error'

def check_time_sync(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "timedatectl | grep 'System clock synchronized'", username, ip, 'health_check')
    try:
...
def run_patch_update(ip, apply_patches=False, username=None, password=None):
    ...
    try:
        ...
        # Log update command
        stdin, stdout, stderr, exit_code = log_command(ssh, cmds['update'], username, ip, 'patch_update')
        artifact_data += f"\nUpdate command output:\n{stdout.read().decode()}\n"
        
        # Log check command
        stdin, stdout, stderr, exit_code = log_command(ssh, cmds['check'], username, ip, 'patch_update')
        ...
        # Log upgrade command
        stdin, stdout, stderr, exit_code = log_command(ssh, cmds['upgrade'], username, ip, 'patch_update')
...
def apply_patches_and_reboot(ip, username, password):
    try:
        ...
        if os_type in ["ubuntu", "debian"]:
            stdin, stdout, stderr, exit_code = log_command(ssh, "[ -f /var/run/reboot-required ] && echo 'reboot required' || echo 'ok'", username, ip, 'patch_and_reboot')
            reboot_required = "reboot required" in stdout.read().decode()
        elif os_type in ["centos", "rhel"]:
            stdin, stdout, stderr, exit_code = log_command(ssh, "needs-restarting -r &> /dev/null; echo $?", username, ip, 'patch_and_reboot')
            reboot_required = stdout.read().decode().strip() == "1"
        elif os_type == "suse":
            stdin, stdout, stderr, exit_code = log_command(ssh, "[ -f /var/run/reboot-needed ] && echo 'reboot required' || echo 'ok'", username, ip, 'patch_and_reboot')
            reboot_required = "reboot required" in stdout.read().decode()
